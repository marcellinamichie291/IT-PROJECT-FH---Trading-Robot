...
def buy_condition(row, previous_row=None):
    readytobuy = False
    if row['ema20'] > row['ema50'] and row['super_trend_direction'] == True:
        readytobuy = True
        if readytobuy == True and previous_row['close'] > previous_row['ema13'] and row['close'] < row['ema13']:
            return True
    else:
        return False
...
def sell_condition(row, previous_row=None):
    ...
    if row['close'] < row['ema50'] and previous_row['close'] < previous_row['ema50']:
        return True
    else:
        return False
